...
const DEFAULT_INLINE_RULES: InlineRule[] = [
  ...
  { name: "italic_underscore", pattern: ITALIC_UNDERSCORE_RE, render: (m, r) => <em>{r(m[1])}</em> },
  { name: "mention", pattern: MENTION_RE, render: (m) => <span data-mention-id={m[1]}>@{m[1]}</span> },
];
...
function tokenizeBlocks(md: string): Token[] {
  ...
  while (i < lines.length) {
    ...
    if (listLines.length) {
      ...
      function build(items: typeof listLines, start = 0, baseIndent = items[0]?.indent ?? 0): { nodes: ListItem[]; end: number } {
        ...
        while (idx < items.length) {
          ...
          const child = childrenLines.length ? build(childrenLines, 0, childrenLines[0].indent) : { nodes: [], end: 0 };
          nodes.push({ content: it.text, children: child.nodes.length ? child.nodes : undefined });
          idx = j;
        }
  ...
  return function renderInline(text: string): ReactNodeish[] {
    ...
    while (remaining.length) {
      ...
      for (const r of rules) {
        ...
        if (m && (earliest === null || m.index < earliest.index)) earliest = { rule: r, match: m, index: m.index };
      }
...
export default function MarkdownRenderer({
  ...
  const inlineRules = useMemo(() => {
    ...
    const mentionRule: InlineRule = components?.Mention
      ? { name: "mention", pattern: MENTION_RE, render: (m) => components.Mention ? React.createElement(components.Mention, { id: m[1] }) : <span data-mention-id={m[1]}>@{m[1]}</span> }
      : { name: "mention", pattern: MENTION_RE, render: (m) => <span data-mention-id={m[1]}>@{m[1]}</span> };

    // final rules: extras first so user can override defaults
    return [...(extraInlineRules ?? []), mentionRule, ...DEFAULT_INLINE_RULES.filter((r) => r.name !== "mention")];
    ...
    return items.map((it, i) => {
      ...
      return (
        <li key={itemKey} className={classMap?.li}>
          ...
          {it.children ? <ul className={classMap?.ul}>{renderList(it.children, itemKey + "-")}</ul> : null}
        </li>
      );
    });
  }
    ...
    <div>
      {tokens.map((t: Token, i: number) => {
        ...
        switch (t.type) {
          ...
          case "list":
            return t.ordered ? (
              <ol key={key} className={classMap?.ol}>{renderList(t.items, String(i) + "-")}</ol>
            ) : (
              <ul key={key} className={classMap?.ul}>{renderList(t.items, String(i) + "-")}</ul>
            );
          case "super_small":
            return (
