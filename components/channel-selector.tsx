...
export default function ChannelSelector({ setSelectedChannel, guildId, disabled }: Props) {
    ...
    const orderedElements = (() => {
        ...
        const hasParentId = (
            c: APIGuildChannel<GuildChannelType>,
        ): c is APIGuildChannel<GuildChannelType> & { parent_id: string | null } => "parent_id" in c;
        ...
        const topLevel = positioned
            .filter((c) => c.type !== ChannelType.GuildCategory && (!hasParentId(c) || c.parent_id === null))
            .sort((a, b) => a.position - b.position);

        const elements: Array<
            | { kind: "channel"; value: (typeof positioned)[number] }
            | { kind: "category"; value: (typeof positioned)[number]; children: (typeof positioned)[number][] }
        > = [];
        ...
        for (const cat of categories) {
            elements.push({
                kind: "category",
                value: cat,
                children: (childrenByParent as Record<string, (typeof positioned)[number][]>)[cat.id] ?? [],
            });
        }
        return elements;
    })();
