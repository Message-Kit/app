import { type APIComponentInContainer, type APIMessageComponent, ComponentType } from "discord-api-types/v10";
...
const validateComponents = (
    ...
    components.forEach((component, noobIndex) => {
        ...
        if (component.type === ComponentType.TextDisplay && component.content.trim() === "") {
            errors.push(`${getComponentName(component.type)} component ${pathStr} is missing content.`);
            ...
            errors.push(`${getComponentName(component.type)} component ${pathStr} must have at least one image.`);
        } else if (component.type === ComponentType.ActionRow && component.components.length <= 0) {
            errors.push(`${getComponentName(component.type)} component ${pathStr} must have at least one button.`);
        }

        if (component.type === ComponentType.Container) {
            if (component.components.length <= 0) {
                errors.push(`${getComponentName(component.type)} ${pathStr} must have at least one component.`);
            } else {
