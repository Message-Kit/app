...
export async function GET() {
    try {
        ...
        if (!botGuilds) {
            return NextResponse.json({ guilds: null, success: false, error: "failed to fetch bot guilds!" });
        }

        const userGuilds = await getUserGuilds();
        if (!userGuilds) {
            return NextResponse.json({ guilds: null, success: false, error: "failed to fetch user guilds!" });
        }
        ...
        mutualGuilds.forEach((mutualGuild) => {
            ...
            const hasAdmin = (perms & BigInt(PermissionFlagsBits.Administrator.toString())) !== BigInt(0);
